...
Router.get('/', (req, res) => {
  ...
  Model.tutorial.findAll('tutorials', ['id', 'DESC'], true).then((snapshot) => {
    if (snapshot.length < 1) {
      ...
      snapshot.forEach((item) => {
        let createdAt = new Memories(),
          updateAt = new Memories(),
          options = {
            prefix: 'birthday',
            suffix: {
              y: 'tahun yang lalu',
        ...
        createdAt.set('datetime', item.dibuat);
        updateAt.set('datetime', item.diupdate);

        if (createdAt.timeAgo() == updateAt.timeAgo()) {
          item.dibuat = createdAt.timeAgo(options.prefix, options.suffix);
          delete item.diupdate;
        } else {
          item.diupdate = updateAt.timeAgo(options.prefix, options.suffix);
          delete item.dibuat;
        }
      });
  ...
  return session.has('superuser')
    ...
    : res.render('admin/index', {
        ...
        message: session.has('message')
          ? JSON.parse(session.get('message'))
          : false,
      });
});
            ...
            if (snapshot.length >= 1) {
              session.flash(
                ...
                JSON.stringify({
                  backgroundStatus: 'bg-red-500',
                  value:
                    'Akun sudah terdaftar, silahkan masuk untuk melanjutkan!',
                })
              );
              res.redirect('/admin/register');
            } else {
                  ...
                  if (!error) {
                    session.flash(
                      ...
                      JSON.stringify({
                        ...
                        value:
                          'Berhasil membuat akun, silahkan masuk untuk melanjutkan!',
                      })
                    );
                    res.redirect('/admin/register');
                  } else {
...
Router.get('/admin/tutorial/edit/:judul', (req, res) => {
  ...
  if (!session.has('superuser')) {
    ...
    Model.tutorial
      .findById('tutorials', [decodeURI(judul), 0])
      .then((snapshot) => {
        if (snapshot.length < 1) {
          res.end(
            'Maaf tutorial ' + decodeURI(judul) + ' tidak dapat ditemukan!'
          );
        } else {
    ...
    Model.tutorial
      ...
      .then((snapshot) => {
        if (snapshot.length < 1) {
          ...
          Model.tutorial.delete('tutorials', snapshot[0].id).then((error) => {
            if (!error) {
              session.flash(
                'message',
                JSON.stringify({
...
Router.post('/admin/tutorial/update', (req, res) => {
  ...
  if (!session.has('superuser')) {
    ...
    return Model.tutorial
      .findById('tutorials', [judul, parseInt(id)])
      .then((snapshot) => {
        return Model.tutorial.update(
          'tutorials',
          {
            judul: judul,
            isi: editor,
